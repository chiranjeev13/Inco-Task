...
import contractAbi from "../artifacts/contracts/PrivateWealth.sol/PrivateWealth.json";
import { HexString } from "@inco/js/dist/binary";
// @ts-ignore
import { Lightning } from '@inco/js/dist/lite';

// Add type for wallet structure
...
describe("PrivateWealth Tests", function () {
  ...
  beforeEach(async function () {
    ...
    for (const [name, userWallet] of Object.entries(namedWallets)) {
      const walletWithAccount = userWallet as WalletWithAccount;
      if (!walletWithAccount.account) {
        console.error(`No account found for wallet ${name}`);
        continue;
      }

      const balance = await publicClient.getBalance({
        address: walletWithAccount.account.address,
      });
      ...
      if (balanceEth < 0.001) {
        ...
        const tx = await wallet.sendTransaction({
          to: walletWithAccount.account.address,
          value: parseEther(neededEth.toFixed(6)),
        });

        await publicClient.waitForTransactionReceipt({ hash: tx });
        console.log(`✅ ${name} funded: ${walletWithAccount.account.address}`);
      }
    }
  });

  describe("Deposit Wealth info", async () => {
    it("Should deposit encrypted wealth", async () => {});
  });
  ...
  //     const encryptedCipherText = await incoConfig.encrypt(
  //       plainTextAmountToBeSent,
  //       {
  ...
  //     );
  //     const aliceWallet = namedWallets.alice as WalletWithAccount;
  //     if (!aliceWallet.account) {
  //       throw new Error("Alice's wallet account is undefined");
  //     }
  //     const txHashForTransfer = await wallet.writeContract({
  //       address: contractAddress,
  //       abi: [transferFunctionAbi],
  //       functionName: "transfer",
  //       args: [aliceWallet.account.address, encryptedCipherText],
  //     });
  ...
  //     const decryptedBalanceForOwnerAfterTransfer =
  //       await reEncryptorForMainWallet({
  ...
  //       })) as HexString;
  //     await new Promise((resolve) => setTimeout(resolve, 1000)); // Wait for the transfer to be processed
  //     const decryptedBalanceForAliceAfterTransfer =
  //       await reEncryptorForAliceWallet({
  ...
  //       args: [aliceWallet.account.address], // Passing Alice's address for decryption
  //     });
